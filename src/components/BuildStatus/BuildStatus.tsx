...
const BuildStatus = (props: BuildStatusProps): JSX.Element => {
  ...
  const buildMessageLogo = isBuildSuccessful ? buildSuccessMessage : buildErrorMessage;

  return (
    <div className="build-status" >
      <img src={buildLogo} className="build-logo" alt={props.statusLogoAlt} />
      <img src={buildMessageLogo} className="build-message-container" alt={props.statusMessageLogoAlt} />
      <h1>{props.heading}</h1>
      <p className="build-message">
          {props.message}
      </p>
    </div>    
  );
